...
class User < ActiveRecord::Base
 ...
 attr_accessible :organisation, :forename, :surname,
 								 ...
 								 :address, :email, :post_code,
 								 :terms_of_service, :telephone
 								 
 validates :organisation, :allow_blank => true,
 													:length => { :within => 4..40 }
 ...
 validates :post_code, :allow_blank => false,
 											 :format => { :with => POST_CODE_REGEX,
 																		:message => "is not a valid uk post code" }											 
	
 validates :telephone, :allow_blank => true,
											 :format => { :with => PHONE_REGEX,
																		:message => "is not a valid uk number" }
